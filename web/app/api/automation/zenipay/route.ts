...
import { generateZeniPayLeads } from "@/src/lib/local-leads";
...
export async function GET(req: NextRequest) {
  ...
  if (!sbOk) await dbLog("zenipay_ai", "Supabase unavailable — using local cache", "warn");

  if (mode === "all" || mode === "generate") {
    await dbLog("zenipay_ai", "Generating ZeniPay leads CAN/US");
    ...
    if (!leads || leads.length === 0) {
      await dbLog("zenipay_ai", "OpenAI unreachable — using local lead generator", "warn");
      leads = generateZeniPayLeads(3) as any[];
    }
    for (const lead of leads) {
      ...
      const { data } = await insertLead({
        ...
        phone: lead.phone, country: lead.country, city: lead.city,
        source: lead.source || "generated", lead_type: "zenipay",
        notes: `Company: ${lead.company}, Type: ${lead.business_type || "business"}, Volume: ${lead.monthly_volume || "unknown"}`,
        outreach_status: "pending", created_at: new Date().toISOString(),
      });
      if (data) results.push({ action: "created", id: data.id, name: data.name, company: lead.company });
    }
    await dbLog("zenipay_ai", `${results.length} leads created`);
  }

  if (mode === "all" || mode === "outreach") {
    await dbLog("zenipay_ai", "Delegating outreach to /api/outreach");
