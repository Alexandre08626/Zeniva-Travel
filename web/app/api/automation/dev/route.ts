...
import { generateDevLeads } from "@/src/lib/local-leads";
...
export async function GET(req: NextRequest) {
  ...
  if (!sbOk) await dbLog("dev_ai", "Supabase unavailable — using local cache", "warn");

  if (mode === "all" || mode === "generate") {
    await dbLog("dev_ai", "Generating dev leads CAN/QC/FR");
    ...
    if (!leads || leads.length === 0) {
      await dbLog("dev_ai", "OpenAI unreachable — using local lead generator", "warn");
      leads = generateDevLeads(3) as any[];
    }
    for (const lead of leads) {
      ...
      const { data } = await insertLead({
        ...
        source: lead.source || "generated", lead_type: "dev",
        notes: `Company: ${lead.company}, Role: ${lead.role}, Tech: ${lead.tech_stack}`,
    ...
    await dbLog("dev_ai", `${results.length} leads created`);
  }

  if (mode === "all" || mode === "outreach") {
    await dbLog("dev_ai", "Delegating outreach to /api/outreach");
