...
export async function POST(req: Request) {
  ...
  try {
    ...
    const activities = (data.activities || []).map((activity: any) => ({
      ...
      startDateTime: from,
      endDateTime: to,
      price: activity.amountsFrom?.[0]?.amount ? `${activity.amountsFrom[0].amount} ${activity.amountsFrom[0].currency}` : "Price on request",
      currency: activity.amountsFrom?.[0]?.currency || 'EUR',
