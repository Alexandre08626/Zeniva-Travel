...
export default function AIAgentsPageClient() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 min-h-screen bg-[#0B1121] rounded-xl">
      {/* Header */}
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-6">
        {[
          ...
          <div key={s.label} className="bg-slate-800/50 border border-slate-700 rounded-lg px-3 py-2 flex items-center gap-2">
      ...
      {/* Tabs */}
      <div className="flex gap-1 mb-6 bg-slate-800/50 rounded-xl p-1 border border-slate-700">
        {TABS.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            ...
            <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-5">
              ...
              <div className="space-y-2.5 max-h-[400px] overflow-y-auto">
                {activity.slice(0, 8).map((a) => (
                  <div key={a.id} className="flex items-start gap-3 bg-slate-800/50 rounded-xl px-3 py-2.5 border border-slate-700/50">
            ...
            <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-5">
              ...
              <div className="space-y-2">
                {agents.map((agent) => {
                  ...
                  return (
                    <div key={agent.id} className="flex items-center gap-3 bg-slate-800/50 rounded-xl px-3 py-2.5 border border-slate-700/50">
          ...
          {approvals.length > 0 && (
            <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-5">
              ...
              <div className="grid md:grid-cols-3 gap-3">
                {approvals.map((ap) => (
                  <div key={ap.id} className="bg-slate-800/50 border border-slate-700 rounded-xl p-3">
      ...
      {tab === "activity" && (
        <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-5">
          ...
          <div className="space-y-2">
            {activity.map((a) => (
              <div key={a.id} className="flex items-start gap-3 bg-slate-800/50 rounded-xl px-4 py-3 border border-slate-700/50">
          ...
          {leads.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {leads.map((lead) => (
                    <tr key={lead.id} className="border-b border-white/5 hover:bg-slate-800/50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
                          ...
                          "bg-slate-800/500/20 text-slate-400"
                        }`}>{lead.status}</span>
                      </td>
                    </tr>
                  ))}
      ...
      {tab === "approvals" && (
        <div className="space-y-4">
          {approvals.length === 0 ? (
            ...
            approvals.map((ap) => (
              <div key={ap.id} className="bg-slate-800/50 border border-slate-700 rounded-2xl overflow-hidden">
                <div className="px-5 py-4 border-b border-white/10 flex items-center justify-between">
                  <div>
      ...
      {tab === "agents" && (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {agents.map((agent) => {
            const sc = STATUS_CONFIG[agent.status];
            return (
              <div key={agent.id} className="bg-slate-800/50 border border-slate-700 rounded-2xl overflow-hidden hover:border-slate-300 transition-all">
                ...
                <div className="px-5 py-4 space-y-3">
                  ...
                  <div className="flex gap-3">
                    {agent.stats.map((s) => (
                      <div key={s.label} className="bg-slate-800/50 rounded-lg px-3 py-1.5 text-center">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {agent.features.map((f) => (
                      <span key={f} className="bg-slate-800/50 text-slate-400 text-[10px] font-medium px-2 py-0.5 rounded-full border border-slate-700/50">{f}</span>
                    ))}
                  </div>
                </div>
              </div>
